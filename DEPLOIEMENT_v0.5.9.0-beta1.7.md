# LTS v0.5.9.0-beta1.7 — Cycles, semaine de tests et navigation Athlète

Cette version remplace `v0.5.9.0-beta1.6`.
...
Le classeur ne nécessite aucune nouvelle feuille ni colonne : les nouvelles écritures utilisent les feuilles existantes `CYCLES` et `TEST_RESULTS`.
...
8. Ouvrir une carte de test, saisir un résultat, synchroniser puis vérifier la ligne correspondante dans `TEST_RESULTS`.
9. Ouvrir une séance Kilter flash et vérifier le relevé des blocs à l’angle prévu.

## Cache PWA

Le cache passe à `lts-pwa-v0.5.9.0-beta1.7`.

Après déploiement, ouvrir une fois :

`https://lts-beta.pages.dev/?v=0590b17`

Si une ancienne interface reste visible, fermer tous les onglets LTS puis rouvrir cette URL.

Message de commit conseillé :

`LTS v0.5.9.0-beta1.7 - cycles test week and athlete day navigation`
